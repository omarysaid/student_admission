        ...
        <div class="row page-titles mx-0" style="background-color: #094469;">
            <div class="col-sm-6 p-md-0">
                <div class="welcome-text">
                    <h4 style="color:white">H.O.Ds Details Table</h4>
                </div>
            </div>
        </div>
        <!-- row -->
                            ...
                            <table id="example" class="display" style="min-width: 845px">
                                ...
                                <tbody>
                                    ...
                                    $select_users = "SELECT u.user_id, u.fullname, u.username, u.gender, u.phone, 
                                                              s.info_id, s.school, s.department, s.course, s.ss_name, 
                                                            s.ss_start_date, s.ss_end_date, s.csee_document, 
                                                            s.as_name, s.as_start_date, s.as_end_date, 
                                                            s.acsee_document, s.birth_certificate , s.status_2
                                                     FROM users u
                                                     INNER JOIN students_info s ON u.user_id = s.user_id
                                                     WHERE u.usertype = 'STUDENT' AND s.status_1 = 1
                                                    ORDER BY s.info_id DESC ";
                                        ...
                                        if ($number > 0) {
                                            while ($row = mysqli_fetch_assoc($result)) {
                                                ...
                                                <tr>
                                                    ...
                                                    <td style="color:black"><?php echo limit_words($row['school'], 3); ?></td>
                                                    <td style="color:black"><?php echo limit_words($row['department'], 3); ?></td>
                                                    <td style="color:black"><?php echo limit_words($row['course'], 3); ?></td>
                                                    ...
                                                    <td style="color:black"><a href="./uploads/<?php echo $row['birth_certificate']; ?>" target="_blank">View</a></td>
                                                     <td style="color:<?php echo $row['status_2'] == 1 ? 'green' : 'red'; ?>"><?php echo $row['status_2'] == 1 ? 'Approved' : 'Not Approved'; ?></td>
                                                    <td>
                                                        <span>
                                                            <a href="../STUDENT/approval2.php?user_id=<?php echo $row['user_id'] ?>">
...
function confirmDelete(Id) {
    ...
    if (confirm("Are you sure you want to delete?")) {
        ...
        window.location.href = "./dup_delete.php?info_id=" + Id;
    }
}
</script>